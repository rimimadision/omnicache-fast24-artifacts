...
/*
 * Use vectored compound I/O command */
//#define PARAFS_VECTOR_CMD
//#define CRFS_FILEBENCH
#define _USE_KMALLOC

#define DEVFS_CACHE

/*
 * Use a global interval tree across host and device
 */
#define GLOBAL_INTERVAL_TREE
...
#define _CFS_SCHED
#define _CFS_SCHED_CPU

#define SCHED_CPU 1
#define SCHED_MEM 2

/* Macro I/O operation related Flags */
#define _MACRO_OPCODE_MASK 0x40

#define DEVFS_READ_APPEND       0x01
#define DEVFS_MACRO_IO_READ     0x02

#define _DEVFS_MACRO_IO_USE_CRC32
/* Use LevelDB checksum */
//#define LEVELDB_OFFLOAD_CHECKSUM
...
#define _DEVFS_QUEUE_PAGES 1048576
...
static void set_bitmap(bitmap_t b, int i) {
        b[i / 8] |= 1 << (i & 7);
}

static void unset_bitmap(bitmap_t b, int i) {
        b[i / 8] &= ~(1 << (i & 7));
}

static uint8_t get_bitmap(bitmap_t b, int i) {
        return b[i / 8] & (1 << (i & 7)) ? 1 : 0;
}

static uint16_t get_set_bits(bitmap_t b) {
...
typedef struct _nvme_command_common {
    ...
    u64                     exec_time;
    u64                     bw_dm_h; // bandwidth of moving data from device memory to host
    u64                     bw_ds_dm; // bandwidth of moving data from device storage to device memory 
} nvme_command_common_t;
...
struct crfss_fstruct {
   ...
   */
   ...
   __u8  fqinit;
	...
	/* Trasaction of current operation */
	void *trans;
...
struct dev_thread_struct {
	...
	int rd_list_miss;
        ...
        /* virtual runtime rbtree for CFS */
        struct rb_root runtime_tree;
...
struct crfss_fstruct* devfs_sched_round_robin_cfs (struct dev_thread_struct *dev_thread_ctx, int sched_type);
...
/* injected crash indicator */
extern int crash_before_inode_log;
extern int crash_in_inode_log;
extern int crash_after_inode_log;
extern int crash_before_balloc;
extern int crash_in_balloc;
extern int crash_after_balloc;
extern int crash_after_checksum;
