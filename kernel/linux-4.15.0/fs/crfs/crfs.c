...
/* Inject crash indicators */
int crash_before_inode_log = 0; 
int crash_in_inode_log = 0; 
int crash_after_inode_log = 0; 
int crash_before_balloc = 0; 
int crash_in_balloc = 0; 
int crash_after_balloc = 0; 
int crash_after_checksum = 0; 
...
//#define CXL_MEM_PG_NUM 1024
//#define CXL_MEM_PG_SIZE (4*1024*1024UL)
//static __u64 cxl_user_addr;
//static __u64 cxl_mem_map[CXL_MEM_PG_NUM];
...
/*
 * Read handler in kernel
 */
long vfio_crfss_io_read_inkernel (struct file *fp , nvme_cmdrw_t *cmdreq, 
			u8 isappend){
 ...
 * read buffer values 
 */
int vfio_crfss_io_append_inkernel (struct file *fp, nvme_cmdrw_t *cmdrw) {
	...
	if (!wrbytes || (wrbytes != cmdrw->nlb)) {
		ret = -EFAULT;
		printk("%s:%u Write failed slba %llu off %llu size %zu\n",
				__FUNCTION__, __LINE__, (loff_t)cmdrw->slba, fpos, wrbytes);
		goto appendio_err;
	}
...
int vfio_crfss_io_append (struct crfss_fstruct *rd, nvme_cmdrw_t *cmdrw) {
	...
	wrbytes = macrofs_xip_file_write(fp, p, cmdrw->nlb, &fpos, (crfs_transaction_t*)rd->trans);
#else
	wrbytes = crfs_xip_file_write(fp, p, cmdrw->nlb, &fpos);
#endif	//_MACROFS_JOURN
 ...
 * read buffer values 
 */
int vfio_crfss_io_write_inkernel(struct file *fp, nvme_cmdrw_t *cmdrw) {
...
inline int vfio_process_compound(struct crfss_fstruct *rd, nvme_cmdrw_t *cmdrw) {
        int retval = 0;

        /* I/O handler on compound ops */
#ifndef PARAFS_VECTOR_CMD
        if (cmdrw->common.opc >= 0x58 && cmdrw->common.opc <= 0x66) {
            retval = vfio_devfs_macro_io_cache_handler(rd, cmdrw);
        } else {
            retval = vfio_devfs_macro_io_handler(rd, cmdrw);
        }
#else
        retval = vfio_devfs_macro_io_parser(rd, cmdrw);
#endif
...
int vfio_creatfile_cmd(unsigned long arg, int kernel_call){
	...
	/*  set fqinit to 0 to skip deallocation as this is call from kernel */
	if (kernel_call) {
		rd->fqinit = 0;
	}
	/*
	 ...
	 * ioctl()
	 */
	if (!kernel_call && map_fd_queue_buffer(rd, map.vaddr)) {
...
/* A lightweight creation of file only to be called from inside the kernel */
int vfio_creatfile_inkernel(char* fname, int flags, umode_t mode, __u8 isjourn){
	...
	fp = crfss_create_file(fname, flags | O_LARGEFILE, mode, &fd);
	if(fp == NULL) {
		printk(KERN_ALERT "%s:%d create file failed \n", 
				__FUNCTION__,__LINE__);
		retval = fd;
...
EXPORT_SYMBOL(vfio_creatfile_inkernel);
	...
	for (i = 0; i < map.cmd_count; i++) {
		...
		if (cmdrw->common.opc == nvme_cmd_write ||
			...
			*/
			...
			//retval =  (long) crfss_llseek (rd,(void *)cmdrw);
                } else if (cmdrw->common.opc == nvme_cmd_compound) {
                        retval = vfio_process_compound(rd, cmdrw);
                } else {
		...
		if (cmdrw->common.opc == nvme_cmd_read) {
			...
			// TODO
                       //retval =  (long) crfss_llseek (rd,(void *)cmdrw);
#ifndef PARAFS_VECTOR_CMD
                } else if ( cmdrw->common.opc & _MACRO_OPCODE_MASK) {
#else
                } else if ( cmdrw->common.opc == nvme_cmd_compound) {
#endif
                        retval = vfio_process_compound(rd, cmdrw);
...
int crfss_close_inkernel(struct file *fp, int fd){

	int retval = 0;

	//fsnotify_close(fp);
	//fput(fp);
#ifdef CRFS_FILEBENCH
	fp->f_pos = 0;
        if (fp->task == NULL) {
		filp_close(fp, NULL);
		return 0;
	}
	__close_fd(fp->task->files, fd);
#else
	fp->f_pos = 0;
	filp_close(fp, NULL);
#endif
...
static int vfio_create_cxl_mem(unsigned long arg){
	...
	cxl_user_addr = start_addr;

	for(i=0; i<map.qpfns; i++) {

		vaddr = start_addr + (CXL_MEM_PG_SIZE * i);
		...
		pud = pud_offset(pgd, vaddr);
		if(pud_none(*pud) || pud_bad(*pud)) {
			printk(KERN_ALERT "DEBUG: Failed %s:%d, idx: %d \n",__FUNCTION__,__LINE__, i);
			retval = -EFAULT;
			break;
		}
		pmd = pmd_offset(pud, vaddr);
		...
		pte_unmap(ptep);
                ...
                cxl_mem_map[i] = (__u64)page_address(page_info);

                if (!cxl_mem_map[i])
                        printk("blk_addr is empty, idx: %d\n", i);
	}
	up_read(&mm->mmap_sem);
...
int vfio_inject_crash(unsigned long arg){

        unsigned long minsz;
        struct vfio_devfs_inject_crash_cmd map;
        int retval = 0, crash_spot = -1;

#if defined(_DEVFS_DEBUG_ENT)
        printk(KERN_ALERT "DEBUG: Calling %s:%d \n",__FUNCTION__,__LINE__);
#endif

        minsz = offsetofend(struct vfio_devfs_inject_crash_cmd, crash_pos);

        if (copy_from_user(&map, (void __user *)arg, minsz)) {
                printk(KERN_ALERT "DEBUG: Failed %s:%d \n",__FUNCTION__,__LINE__);
                retval = -EFAULT;
                goto err_inject_crash;
        }
        crash_spot = map.crash_pos;
        switch (crash_spot) {
                case CRASH_BEFORE_INODE_LOG:
                        crash_before_inode_log = 1;
                        break;
                case CRASH_IN_INODE_LOG:
                        crash_in_inode_log = 1;
                        break;
                case CRASH_AFTER_INODE_LOG:
                        crash_after_inode_log = 1;
                        break;
                case CRASH_BEFORE_BALLOC:
                        crash_before_balloc = 1;
                        break;
                case CRASH_IN_BALLOC:
                        crash_in_balloc = 1;
                        break;
                case CRASH_AFTER_BALLOC:
                        crash_after_balloc = 1;
                        break;
                case CRASH_AFTER_CHECKSUM:
                        crash_after_checksum = 1;
                        break;
                default:
                        break;
        }
...
long crfss_ioctl(void *iommu_data, unsigned int cmd, unsigned long arg)
{
	...
	if (cmd == VFIO_IOMMU_GET_QUEUE_ADDR) {
        ...
        } else if (cmd == VFIO_IOMMU_GET_CXL_MEM_ADDR) {
		ret = vfio_create_cxl_mem(arg);
	...
	} else if (cmd == VFIO_DEVFS_INJECT_CRASH_CMD) {
                ret = vfio_inject_crash(arg);
        }
