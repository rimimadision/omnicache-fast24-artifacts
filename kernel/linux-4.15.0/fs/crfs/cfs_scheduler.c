...
int update_queue_sched_res(struct crfss_fstruct *rd, nvme_cmdrw_t *cmdrw, int sched_type) {
        long retval = 0;
        switch(sched_type) {
                case SCHED_CPU:
...
struct crfss_fstruct* rb_search(struct rb_root* root, unsigned long vir_runtime) {
    struct rb_node* node = root->rb_node;

    while (node) {
        struct crfss_fstruct* queue = rb_entry(node, struct crfss_fstruct, rb);
        int64_t result;
...
int rb_insert(struct rb_root* root, struct crfss_fstruct* queue, int sched_type) {
    struct rb_node** new = &(root->rb_node), *parent = NULL;

    /* Figure out where to put new node */
    while (*new) {
        struct crfss_fstruct* this = rb_entry(*new, struct crfss_fstruct, rb);
        int64_t result = get_sched_res(queue, sched_type) - get_sched_res(this, sched_type);
...
struct crfss_fstruct* devfs_sched_round_robin_cfs
	...
	if (dev_thread_ctx->current_rd_list &&
		dev_thread_ctx->current_rd_list->next != (&dev_thread_ctx->dummy_rd.list)) {
		...
		rcu_read_lock();
		list_for_each_entry_continue_rcu(rd, &dev_thread_ctx->dummy_rd.list, list) {
			if (!rd || IS_ERR(rd)) {
			...
			if (sched_res <= min_sched_res && test_and_set_bit(0, &rd->state) == DEVFS_RD_IDLE) {
	...
	rcu_read_unlock();
        ...
        if (rd != NULL) {
                ...
                if (cmdrw != NULL) {
                        ...
                        rb_insert(&dev_thread_ctx->runtime_tree, rd, sched_type);
#ifdef _USE_GMALLOC
                        if (sched_type == SCHED_MEM) {
                                crfss_mutex_lock(&mm_mutex);
                                if (g_mm_counter + cmdrw->mm_budget > g_mm_budget) {
