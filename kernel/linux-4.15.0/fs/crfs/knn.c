...
void read_knn_data(struct crfss_fstruct *rd, nvme_cmdrw_t *cmdrw_mop) {
    ...
    while (1) {
        ...
        while (read_len != READ_BUF_SIZE ) {
            ...
            cmdrw_mop->common.prp2 = (uint64_t)buf;
            cmdrw_mop->slba = DEVFS_INVALID_SLBA;
            cmdrw_mop->nlb = READ_BUF_SIZE - read_len;

            isappend = (cmdrw_mop->slba == DEVFS_INVALID_SLBA) ? 1 : 0;

            tmp_len = vfio_crfss_io_read(rd, cmdrw_mop, isappend);
            if (tmp_len <= 0)  {
                
                printk("break, tmp_len: %ld\n", tmp_len);
                break;
            }
            printk("vfio read, ret: %ld\n", tmp_len);
            read_len += tmp_len;
        }
        if (!tmp_len) {
            break;
        ...
        for (k = 0; k < READ_BUF_SIZE / BUS_WIDTH; k++) {
            ...
            if (state == 0) {
                ...
                for (i = 0; i < BUS_WIDTH / WEIGHT_SIZE; i++) {
                    ...
                    train_cases[data_idx].feature_vec[feature_idx++] = weight;
                }
                if (state == FEATURE_DIM * WEIGHT_SIZE / BUS_WIDTH) {
                    printk("feature_idx: %ld, FEATURE_DIM: %ld\n", feature_idx, FEATURE_DIM);
                    BUG_ON(feature_idx != FEATURE_DIM);
                    state = 0;
                    feature_idx = 0;
                    data_idx++;
                } else {
	...
	BUG_ON(data_idx != NUM_TRAIN_CASES);
}
...
void gen_predicting_data(PredictingCase* predicting_cases) {
  ...
  for (i = 0; i < NUM_PREDICTING_CASES; i++) {
    for (j = 0; j < FEATURE_DIM; j++) {
      ...
      predicting_cases[i].feature_vec[j] = MAX_FEATURE_WEIGHT;
    }
  }
}
...
void prediction(unsigned int* distance_arr) {
  ...
  unsigned int* freq_map = vmalloc(NUM_TRAIN_CASES*sizeof(unsigned int));
