...
static int crfs_get_block_info(struct super_block *sb,
...
#if 1   // The below code is just a temporary fix of running with real
        // Intel PM
        if (ret == 1)
                --ret;
#endif
...
int crfs_fill_super(struct super_block *sb, void *data, int silent)
{
	...
	/* Init a new crfs instance */
	if (sbi->vmmode) {
                /* 
		 * Mount in VM mode, that the entire FS space is a 64MB
		 * memory region by vmalloc().
		 * the reason we need VM mode is that it is convenient
		 * to debug our file system in QEMU
		 */
		if (!fs_initialized) {
			/* 
			 * qemu_vaddr not set means the file system has not been
			 * initialized yet, so call pmfs_init() to initialize a
			 * new instance and format the storage
			 */
			...
			/* Mark file system as initialized */
			fs_initialized = true;

			goto setup_sb;
		} else {
			/* If file system is already set before, then do not format the storage */
			if (!qemu_vaddr) {
				printk(KERN_ALERT "qemu_vaddr is NULL!\n");
				goto out;
			}
			sbi->virt_addr = qemu_vaddr;
        ...
        if (sbi->s_mount_opt & PMFS_MOUNT_FORMAT) {
                printk(KERN_ERR "%s:%d Before crfs_init\n",__FUNCTION__,__LINE__);
                root_pi = crfs_init(sb, sbi->initsize);
                printk(KERN_ERR "%s:%d After crfs_init\n",__FUNCTION__,__LINE__);
                if (IS_ERR(root_pi))
                        goto out;
                super = crfs_get_super(sb);
