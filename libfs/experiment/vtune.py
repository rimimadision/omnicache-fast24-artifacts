...
def execute_run(test_folder, run, debugger):
    ...
    # 7. Run with original command
    if "origin" in analysis:
        clear_pagecache(debug)
        debug("output folder: " + run_folder)
        output_file = os.path.join(run_folder, run["folder"] + "_output.txt")
        command = run["command"] + " 2>&1 | tee -a " + output_file
        debug("Running command: " + command)
        start_time = time.time()
        commands.getstatusoutput(command)
        debug("Ran in %s seconds" % (time.time() - start_time))
