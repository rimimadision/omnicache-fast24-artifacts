...
# Define the arrays
cachesize_ratio = ["25%", "50%", "75%", "100%"]
iosize_arr = [1, 2, 3, 4]
omnicahche_iosize_arr = [1, 3, 5, 8]
...
base_dir_template = f"{output_dir}/microbench/figure7/{{config}}/{{workload}}/{{thd}}/"
...
def main():
    with open(output_file, mode='w', newline='') as csv_file:
        ...
        for access_pattern in workload_arr:
            ...
            header_row = ["CacheSize"] + config_out_arr
            csv_writer.writerow(header_row)
            ...
            for workload in workload_arr:
                ...
                for thd in iosize_arr:

                    workload_data = [cachesize_ratio[thd-1]]
                    for config in config_arr: 

                        #print(thd)
                        if config == "omnicache":
                            path_thd = omnicahche_iosize_arr[thd-1]
                        elif config == "fusionfs":
                            path_thd = 4
                        else:
                            path_thd = thd

                        base_dir = base_dir_template.format(workload=workload, config=config, thd=path_thd)
                        #print(base_dir)
