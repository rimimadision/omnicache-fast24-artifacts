...
RUN() {
    ...
    #numactl --physcpubind=0-15,32-47 --membind=0 $MICROBENCH/build/test_smart_cache_posix_hybrid $2 4096 $1 $FILESIZE $FILENUM #&> $RESULTDIR/"fusionfs_cachesize_"$2"_threads_"$1"_filenum_"$FILENUM"_filesize_"$FILESIZE".txt" 

    numactl --physcpubind=16-31,48-63 $MICROBENCH/build/test_smart_cache_posix_hybrid $2 1024 $1 $FILESIZE $FILENUM &>> SEQ-OMNICACHE.out
