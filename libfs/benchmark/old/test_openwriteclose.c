...
//    printf("create filename: %s \n", filename);
    ...
    /* printf("fd: %d, file: %s, finish writing %lu blocks\n", fd, filename, i); */
#ifndef _POSIX
    crfsclose(fd);
#else
    close(fd);
...
void* do_open_write_close(void *arg) {
        ...
        while ((entry = readdir(mydir)) != NULL) {
                ...
                /* printf("open write close, %s, blocks: %d\n", filearr,FILESIZE/BLOCKSIZE); */
                if (fd > 0)
#ifdef _POSIX
                        close(fd);
#else
...
void* do_open_write_close_batch(void *arg) {
...
void* do_open_write_close_batch(void *arg) {
        ...
        char test_file[256];
        bzero(test_file, 256);
        snprintf(test_file, sizeof(test_file), "%s%d", TESTDIR "/testfile", thread_id);
        printf("test_file: %s\n", test_file);
        ...
        gettimeofday(&start_t, NULL);
        for (int j = 0; j < FILENUM / thread_nr; j+= batch_size) {

                char filename[256];
                snprintf(filename, sizeof(filename), "%s/%s%d", read_dir, "testfile", j);
                /* printf("create file, filename: %s, blocks: %d\n", filename, ITERS/ FILENUM); */
                for (int k = 0; k < batch_size; k++) {
                        memset(bufs[k], 0x61 + j % 26, FILESIZE);
                }

                if (devfsopenpwriteclose_batch(tmp_fd, (const void **)bufs, FILESIZE, 0, filename, batch_size) != FILESIZE) {
...
void* do_open_write_close(void *arg) {
        ...
        for (int j = 0; j < FILENUM / thread_nr; j++) {
                ...
                if (devfsopenpwriteclose(tmp_fd, buf, FILESIZE, 0, filename) != FILESIZE) {
                        printf("File data block read fail \n");
                        return NULL;
                }
...
int main(int argc, char **argv) {
	...
	if (argc < 4) {
		...
		printf("./test_openwritelose file_num file_size(MB) thread_count batch\n");
		return 0;
	}
	...
	/* Step 1: Create Testing File */
        for (i = 0; i < thread_nr; i++) {
                ...
                for (int j = 0; j < FILENUM / thread_nr; j++) {
                        ...
                        char filename[256];
                        snprintf(filename, sizeof(filename), "%s/%s%d", read_dir, "testfile", j);
                        /* printf("create file, filename: %s, blocks: %ld\n", filename, FILESIZE / BLOCKSIZE); */
	...
	for (i = 0; i < thread_nr; ++i) {
		thread_idx[i] = i;
                if (batch) {
                        printf("batch\n");
		        pthread_create(&tid[i], NULL, do_open_write_close_batch, &thread_idx[i]);
                } else {
		        pthread_create(&tid[i], NULL, do_open_write_close, &thread_idx[i]);
                }
	}
