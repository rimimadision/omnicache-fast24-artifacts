...
load_workload()
{
    ...
    LD_PRELOAD=$DEVFSCLIENT/libshim/shim_common.so $leveldb_build_dir/db_bench_hostcache --use_existing_db=0 --benchmarks=ycsb --db=$database_dir --threads=1 --open_files=1000 2>&1 | tee $fs_results/run$run_id
...
run_workload()
{
    ...
    $leveldb_build_dir/db_bench_hostcache --use_existing_db=1 --benchmarks=ycsb --db=$database_dir --threads=1 --open_files=1000 2>&1 | tee $fs_results/run$run_id
